 ...
 *             Mudar cor do circulo com teclas          *
 *             Mostra um 'png'                          *
 *                                                      *
 *      GTK+ 3                                          *
 *      Versao: 1.0                                     *
	      ...
	      gpointer    data   )
{
  ...
  cairo_show_text  (cr, "* Para mudar a cor do circulo prima as teclas");

  cairo_move_to(cr, 90, 50);
  cairo_show_text  (cr, "(r ... R) - Vermelho");
  cairo_move_to(cr, 90, 70);
  cairo_show_text  (cr, "(g ... G) - Verde");
  cairo_move_to(cr, 90, 90);
  cairo_show_text  (cr, "(b ... B) - Azul");
  cairo_move_to(cr, 260, 50);
  cairo_show_text  (cr, "(a, A) - Preto");
  cairo_move_to(cr, 260, 70);
  cairo_show_text  (cr, "(z, Z) - Branco");
...
cb_key_press (GtkWidget    *widget   ,
              ...
              ProgData     *pinfo    )
{
  //char   *str1  = gdk_keyval_name (eventkey->keyval);
  //printf ("Key Press - %d - %s\n", eventkey->keyval, str1);

  switch (eventkey->keyval)
    {
    case 'R' : change_color (pinfo, 0, 0.05); break;
    case 'r' : change_color (pinfo, 0, -0.05); break;
    case 'G' : change_color (pinfo, 1, 0.05); break;
    case 'g' : change_color (pinfo, 1, -0.05); break;
    case 'B' : change_color (pinfo, 2, 0.05); break;
    case 'b' : change_color (pinfo, 2, -0.05); break;
    case 'a' : case 'A' : pinfo->rgb[0] = pinfo->rgb[1] = pinfo->rgb[2] = 0.; break;
    case 'z' : case 'Z' : pinfo->rgb[0] = pinfo->rgb[1] = pinfo->rgb[2] = 1.; break;
    default  : break;
    }
...
int main(int argc, char *argv[])
{
  ...
  g_signal_connect (G_OBJECT(pinfo->window), "button-press-event", 
		    G_CALLBACK (cb_motion_notify), pinfo);
  g_signal_connect (G_OBJECT(pinfo->window), "button-release-event", 
		    G_CALLBACK (cb_motion_notify), pinfo);
  g_signal_connect (G_OBJECT(pinfo->window), "key-press-event", 
		    G_CALLBACK (cb_key_press), pinfo);
