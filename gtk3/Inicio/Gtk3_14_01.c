...
gchar     *winTitle    = "GTK3 - Check Buttons"  ;
	 	  ...
	 	  GtkWidget  *bt )
{
  if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(bt)))
    {
      gtk_label_set_text (GTK_LABEL (label2), "B: Agora estou desactivado!");
...
main (int argc, char **argv)
{
  ...
  button1 = gtk_check_button_new_with_label ("Eu sou a 'Escolha' A");
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON(button1), FALSE);
  gtk_box_pack_start (GTK_BOX(vbox), button1, FALSE, TRUE, 3);

  button2 = gtk_check_button_new_with_label ("Eu sou a 'Escolha' B");
  gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON(button2), TRUE);
  gtk_box_pack_start (GTK_BOX(vbox), button2, FALSE, TRUE, 3);
