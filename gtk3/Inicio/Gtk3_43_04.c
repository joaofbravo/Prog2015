...
{      
  gchar          texto[128] ;
  gdouble        pos1x = 130., pos1y = 250.;
  gdouble        pos2x = 500., pos2y = 400;
  ...
  sprintf (texto, "Keys [Scale: (%.1lf,%.1lf) ; Trans: (%3.0lf,%3.0lf)]:", 
	   scale1x, scale1y, trans1x, trans1y);
  cairo_move_to (cr, 10, 100);
  cairo_show_text (cr, texto);

  strcpy (texto, "Scale H:       (g .. h) -+ 0.1");
  cairo_move_to (cr, 30, 120);
  cairo_show_text (cr, texto);

  strcpy (texto, "Scale V:       (c .. v) += 0.1");
  ...
  cairo_move_to (cr, 30, 190);
  cairo_show_text (cr, texto);

  strcpy (texto, "Reset: (r) [ 1.0 ; 0.0 ]");
  cairo_move_to (cr, 30, 220);
  cairo_show_text (cr, texto);
	      ...
	      GtkWidget    *darea    )
{
  switch (eventkey->keyval)
    ...
    case 'r' : case 'R' : scale1x = scale1y = 1.0; trans1x = trans1y = 0; break;
