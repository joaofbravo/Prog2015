...
create_ComboBox_Text (void)
{
  ...
  //  gtk_widget_set_size_request (cbBox, 152, 29);
  gtk_widget_set_size_request (cbBox, 52, 29);

  for (i1 = 0 ; i1 < CBOX_QT ; ++i1)
    gtk_combo_box_text_insert (GTK_COMBO_BOX_TEXT (cbBox), i1, NULL, cb_text[i1]);
  //gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (cbBox), cb_text[i1]);     // Variante
