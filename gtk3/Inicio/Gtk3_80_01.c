...
void
gtk_read_css (ProgInfo  *pinfo , 
	      gchar     *fcss  )
{
  pinfo->provider = gtk_css_provider_new ();
  pinfo->display = gdk_display_get_default ();
  pinfo->screen = gdk_display_get_default_screen (pinfo->display);
  gtk_style_context_add_provider_for_screen (pinfo->screen, GTK_STYLE_PROVIDER (pinfo->provider),
					     GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
  gtk_css_provider_load_from_path (pinfo->provider, fcss, NULL);
...
{
  GtkWidget  *mbox, *hbox2, *vbox0, *vbox ; // *label, 
  ProgInfo   *pinfo ;
  ...
  //gtk_container_add (GTK_CONTAINER (vbox0), mbox);
  ...
  gtk_widget_set_size_request (bt2, 130, 25);
